...
class DataPersistenceManager {
  ...
  calculateCompletedSteps(progressData) {
    return Object.values(progressData.steps || {})
...
class GlobalProgressManager extends DataPersistenceManager {
  ...
  updateGlobalProgress() {
    ...
    allGames.forEach(gameId => {
      ...
      if (gameProgress && Object.keys(gameProgress.steps || {}).length > 0) {
  ...
  aggregateGameData(globalData, gameProgress, gameId) {
    ...
    const completionPercentage = this.calculateCompletionPercentage(gameProgress);
    ...
    if (gameProgress.pokemon) {
      Object.entries(gameProgress.pokemon).forEach(([pokemonName, pokemonData]) => {
        ...
        if (pokemonData.catches && pokemonData.catches.length > 0) {
          ...
          pokemonData.catches.forEach(catchData => {
            globalData.pokedex[pokemonName].locations.add(catchData.location);
          });
        }
        
        globalData.pokedex[pokemonName].timesEncountered += pokemonData.totalEncounters || 0;
      });
    }

    // Aggregate other statistics
    globalData.stats.totalItemsCollected += Object.keys(gameProgress.items || {}).length;
    globalData.stats.totalBattlesWon += Object.keys(gameProgress.battles || {}).length;
  }
  ...
  getLivingPokedexStatus() {
    ...
    const caughtPokemon = Object.values(pokedex).filter(pokemon => 
      pokemon.caught && pokemon.caught.length > 0
    );
  ...
  getMissingPokemon(pokedex) {
    ...
    return allPokemon.filter(pokemon => 
      !pokedex[pokemon] || !pokedex[pokemon].caught || pokedex[pokemon].caught.length === 0
    );
  }
}
```
...
class BackupManager extends GlobalProgressManager {
  ...
  restoreFromBackup(dataKey, backupTimestamp = null) {
    try {
      ...
      let targetBackup;
      if (backupTimestamp) {
        targetBackup = backupKeys.find(key => key.includes(backupTimestamp));
      } else {
...
class DataExportImportManager extends BackupManager {
    ...
    try {
      ...
      allGames.forEach(gameId => {
        ...
        if (gameData && Object.keys(gameData.steps || {}).length > 0) {
          ...
          exportData.metadata.completedSteps += gameData.completedSteps || 0;
        }
      });
    ...
    try {
      ...
      const exportData = {
        ...
        metadata: {
          ...
          completionPercentage: this.calculateCompletionPercentage(gameData)
        }
      };

      return this.generateExportFile(exportData, `pokemon-walkthrough-${gameId}`);
    } catch (error) {
  ...
  validateImportData(data) {
    ...
    const requiredFields = ['exportVersion', 'exportTimestamp', 'exportType'];
    ...
    if (data.exportType === 'complete') {
      return data.hasOwnProperty('games') && typeof data.games === 'object';
    ...
    try {
      ...
      Object.entries(importData.games).forEach(([gameId, gameData]) => {
        try {
          ...
          importResults.errors.push(`Failed to import ${gameId}: ${error.message}`);
        }
      });
      ...
      if (importData.preferences) {
        this.save(STORAGE_KEYS.USER_PREFERENCES, importData.preferences);
      }
  ...
  importGameData(importData) {
    try {
      const success = this.saveGameProgress(importData.gameId, importData.data);
...
class StorageOptimizer extends DataExportImportManager {
  ...
  getStorageUsage() {
    ...
    for (let key in localStorage) {
      if (localStorage.hasOwnProperty(key) && key.startsWith('pokemon_walkthrough')) {
    ...
    if (error.name === 'QuotaExceededError') {
      ...
      if (cleanupResult.cleanedItems > 0) {
        ...
        try {
          ...
          console.info(`Save succeeded after cleaning ${cleanupResult.cleanedItems} items`);
          return true;
        } catch (retryError) {
...
class DataMigrationManager extends StorageOptimizer {
  ...
  migrateLegacyData(data) {
    ...
    console.info(`Migrated ${Object.keys(migratedData.steps).length} steps from legacy format`);
    return migratedData;
  }

  migrateToCurrentVersion(data, fromVersion) {
    console.info(`Migrating data from version ${fromVersion} to ${this.currentVersion}`);
  ...
  performBulkMigration() {
    ...
    console.info(`Starting migration for ${migrationStatus.gamesNeedingMigration.length} games`);
    ...
    migrationStatus.gamesNeedingMigration.forEach(({ gameId, currentVersion }) => {
      try {
        console.info(`Migrating ${gameId} from version ${currentVersion}`);
    ...
    console.info(`Migration completed: ${results.migratedGames} games migrated, ${results.errors.length} errors`);
    return results;
  }
}
```
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (migrationStatus.needsMigration) {
    ...
    if (results.success) {
      console.info(`Migration completed successfully for ${results.migratedGames} games`);
    } else {
...
setInterval(() => {
  ...
  if (parseFloat(usage.percentUsed) > 80) {
    ...
    if (cleanupResult.cleanedItems > 0) {
      console.info(`Cleaned up ${cleanupResult.cleanedItems} items, reclaimed ${cleanupResult.reclaimedSpaceKB}KB`);
    }
  }
