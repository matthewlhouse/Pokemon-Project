...
class ProgressManager extends EventEmitter {
    ...
    async loadGame(gameId) {
        ...
        this.gameProgress =
            this.loadFromStorage(`game_${gameId}`) ||
            this.getDefaultGameProgress(gameId);
    ...
    uncheckStep(stepId) {
        if (this.gameProgress.steps[stepId]) {
            ...
            this.gameProgress.steps[stepId].timestamp =
    ...
    handlePokemonCaught(pokemonName, stepData) {
        ...
        if (!this.gameProgress.pokemon[pokemonName].firstCaught) {
            this.gameProgress.pokemon[pokemonName].firstCaught =
                stepData.location;
        }
        ...
        if (progressBar) {
            ...
            if (progressText) {
                ...
                progressText.textContent = `${Math.round(
                    percentage
                )}% Complete (${completed} of ${total} steps)`;
            }
        }
    ...
    updateLocationCounters() {
        ...
        locations.forEach(location => {
            ...
            const counter = location.querySelector(
                '.step-counter .counter-number'
            );
            const counterText = location.querySelector(
            ...
            if (counter && counterText) {
                ...
                if (remaining === 0) {
                    ...
                    counterText.textContent =
                        remaining === 1 ? 'step remaining' : 'steps remaining';
    ...
    applyProgressToDOM() {
        ...
        Object.entries(this.gameProgress.steps || {}).forEach(
            ([stepId, stepData]) => {
        ...
        // Apply saved choices
        Object.entries(this.gameProgress.choices || {}).forEach(
            ([choiceType, value]) => {
                const input = document.querySelector(
                    `input[data-choice="${choiceType}"][value="${value}"]`
                );
                if (input) {
                    input.checked = true;
                }
    ...
    saveProgress() {
        ...
        try {
            ...
            this.eventBus.emit(EVENT_NAMES.PROGRESS_SAVED, {
...
class FilterManager extends EventEmitter {
    ...
    initializeUI() {
        ...
        const displayModeInputs = document.querySelectorAll(
    ...
    applyFilters() {
        ...
        steps.forEach(step => {
            const stepTags = step.dataset.tags
                .split(',')
                .map(tag => tag.trim());
    ...
    updateLocationVisibility() {
        ...
        locations.forEach(location => {
            const visibleSteps = location.querySelectorAll(
            ...
            if (visibleSteps.length === 0) {
                ...
                if (counter) {
                    const completedVisible = Array.from(visibleSteps).filter(
                        step => step.classList.contains('completed')
                    ).length;
                    counter.textContent =
                        visibleSteps.length - completedVisible;
                }
            }
        });
    }
...
class UIManager extends EventEmitter {
    ...
    handleStepToggle(checkbox) {
        ...
        if (stepElement.dataset.pokemon) {
            const levelInput = stepElement.querySelector(
                '[data-field="level"]'
            );
            const encountersInput = stepElement.querySelector(
            ...
            stepData.pokemon = {
                ...
                location: stepElement.dataset.location,
                encountersBeforeCatch: encountersInput?.value
                    ? parseInt(encountersInput.value)
                    : 1,
            };
        }
    ...
    showStepCompletionFeedback(stepElement, completed) {
        ...
        const stepText =
            stepElement.querySelector('.step-text')?.textContent || 'Step';
    ...
    announceRegionToggle(summary) {
        ...
        const locationName =
            summary.querySelector('.location-name')?.textContent || 'Section';
