...
class ContentValidator {
  constructor() {
    this.rules = {
      ...
      qualityChecks: {
        ...
        categoryWhitelist: ['story', 'pokemon', 'trainer-battle', 'gym-battle', 'item', 'progression', 'choice', 'setup']
      }
    };
  }
  ...
  validateTags(stepElement, errors, warnings) {
    ...
    if (tags.length < this.rules.qualityChecks.tagsPerStep.min) {
      warnings.push(`Step has only ${tags.length} tags, recommend at least ${this.rules.qualityChecks.tagsPerStep.min}`);
    } else if (tags.length > this.rules.qualityChecks.tagsPerStep.max) {
      warnings.push(`Step has ${tags.length} tags, consider reducing to max ${this.rules.qualityChecks.tagsPerStep.max}`);
    }
  }
  ...
  validateContentQuality(stepElement, warnings) {
    ...
    if (stepText.length < this.rules.qualityChecks.stepTextLength.min) {
      warnings.push(`Step text is very short (${stepText.length} chars) - may need more detail`);
    } else if (stepText.length > this.rules.qualityChecks.stepTextLength.max) {
      warnings.push(`Step text is very long (${stepText.length} chars) - consider breaking into multiple steps`);
    }
    ...
    if (!stepText.match(/^(Go|Talk|Battle|Catch|Choose|Enter|Exit|Find|Get|Use|Heal|Buy|Sell)/i)) {
  ...
  validateDataConsistency(stepElement, errors, warnings) {
    ...
    if (category && !this.rules.qualityChecks.categoryWhitelist.includes(category)) {
      errors.push(`Invalid category: ${category}. Must be one of: ${this.rules.qualityChecks.categoryWhitelist.join(', ')}`);
    }
...
class WritingStyleChecker {
  constructor() {
    this.styleRules = {
      // Good patterns
      goodPatterns: [
        { pattern: /^(Go|Talk|Battle|Catch|Choose|Enter|Exit|Find|Get|Use|Heal|Buy|Sell|Visit|Approach|Leave|Return)/, message: 'Starts with action verb' },
        { pattern: /\b(you|your)\b/i, message: 'Uses player-focused language' }
      ],
      
      // Avoid patterns
      avoidPatterns: [
        { pattern: /\b(obviously|clearly|simply|just|easily)\b/i, message: 'Avoid assumption words' },
        { pattern: /\b(guys?|dudes?)\b/i, message: 'Use inclusive language' },
        { pattern: /\bembarl on\b/i, message: 'Avoid flowery language' },
        { pattern: /\b(tons of|lots of|a bunch of)\b/i, message: 'Use specific quantities' }
      ],
  ...
  checkText(text) {
    ...
    if (!hasGoodPattern) {
      issues.push({ type: 'style', severity: 'warning', message: 'Consider starting with an action verb' });
    }
...
class PokemonDataChecker {
  ...
  validatePokemonData(pokemonName, providedData) {
    ...
    if (!canonicalData) {
      warnings.push(`Pokemon "${pokemonName}" not found in database - manual verification needed`);
      return { errors, warnings };
    }
    
    // Validate National Dex number
    if (providedData.nationalDex && providedData.nationalDex !== canonicalData.nationalDex) {
      errors.push(`Incorrect National Dex number: expected ${canonicalData.nationalDex}, got ${providedData.nationalDex}`);
    }
    ...
    if (providedData.types) {
      ...
      if (JSON.stringify(expectedTypes) !== JSON.stringify(providedTypes)) {
        errors.push(`Incorrect types: expected ${expectedTypes.join('/')}, got ${providedTypes.join('/')}`);
      }
    }
...
class ContentMaintenanceWorkflow {
  ...
  getSuggestionForErrorType(type) {
    const suggestions = {
      'missing-attributes': 'Review HTML patterns and ensure all steps include required data attributes',
