...
class ContentValidator {
  constructor() {
    ...
    this.validCategories = ['story', 'pokemon', 'trainer-battle', 'gym-battle', 'item', 'progression', 'choice', 'setup'];
    this.validTags = [
      'required', 'optional', 'story-critical', 'recommended', 'missable',
      'pokemon-encounter', 'wild-pokemon', 'trainer-battle', 'gym-battle',
      'item-collection', 'hidden-item', 'key-item', 'preparation', 'tutorial',
      'version-red', 'version-blue', 'version-yellow', 'spoiler-free',
  ...
  validateHTML(document, filePath) {
    ...
    if (!metaViewport) {
      this.addWarning(filePath, 'HTML Structure', 'Missing viewport meta tag (recommended for responsive design)');
    }
  }
  ...
  validateStepAttributes(step, stepId, filePath) {
    this.requiredAttributes.forEach(attr => {
      if (!step.hasAttribute(attr)) {
        this.addError(filePath, 'Step Attributes', `Step missing required attribute: ${attr}`, stepId);
      }
    });
  }
  ...
  validateStepIdentifiers(stepId, category, location, filePath) {
    ...
    if (stepId && !this.stepIdNaming.test(stepId)) {
      this.addError(filePath, 'Step ID', `Step ID "${stepId}" should use kebab-case (lowercase letters, numbers, hyphens only)`, stepId);
    }

    // Validate category
    if (category && !this.validCategories.includes(category)) {
      this.addError(filePath, 'Step Category', `Invalid category "${category}". Valid categories: ${this.validCategories.join(', ')}`, stepId);
    }

    // Validate location format
    if (location && !this.locationNaming.test(location)) {
      this.addError(filePath, 'Location ID', `Location ID "${location}" should use kebab-case`, stepId);
    }
  }
  ...
  validateTagConflicts(tagList, stepId, filePath) {
    if (tagList.includes('required') && tagList.includes('optional')) {
      this.addError(filePath, 'Tag Conflict', 'Step cannot be both "required" and "optional"', stepId);
    }
  }
  ...
  validateTagValues(tagList, stepId, filePath) {
    tagList.forEach(tag => {
      if (!this.validTags.includes(tag)) {
        this.addWarning(filePath, 'Unknown Tag', `Unknown tag "${tag}" - consider adding to valid tags list`, stepId);
      }
    });
  }
  ...
  validateTagCount(tagList, stepId, filePath) {
    if (tagList.length < 2) {
      this.addWarning(filePath, 'Tag Count', 'Step should have at least 2 descriptive tags', stepId);
    }
    if (tagList.length > 8) {
      this.addWarning(filePath, 'Tag Count', 'Step has many tags - consider consolidating', stepId);
    }
  }
  ...
  validateStepContent(step, stepId, filePath) {
    ...
    if (textLength < 10) {
      this.addWarning(filePath, 'Step Content', 'Step text is very short - may need more detail', stepId);
    }
    
    if (textLength > 200) {
      this.addWarning(filePath, 'Step Content', 'Step text is very long - consider breaking into multiple steps', stepId);
    }
  }
  ...
  validateStepAccessibility(step, stepId, filePath) {
    ...
    if (checkboxId && labelFor && checkboxId !== labelFor) {
      this.addError(filePath, 'Form Accessibility', `Checkbox ID "${checkboxId}" doesn't match label for "${labelFor}"`, stepId);
    }
  }
  ...
  validatePokemonStep(step, stepId, filePath) {
    ...
    if (!pokemonAttr) {
      this.addWarning(filePath, 'Pokemon Data', 'Pokemon step should have data-pokemon attribute', stepId);
    }
  }
  ...
  validateBattleStep(step, stepId, filePath) {
    ...
    if (!trainerAttr) {
      this.addWarning(filePath, 'Battle Data', 'Battle step should have data-trainer attribute', stepId);
    }
  }
  ...
  validateLocation(location, filePath) {
    ...
    if (!locationId) {
      this.addError(filePath, 'Location Attributes', 'Location section missing data-location-id attribute');
      return;
    }

    if (!this.locationNaming.test(locationId)) {
      this.addError(filePath, 'Location ID', `Location ID "${locationId}" should use kebab-case`);
    }

    // Check for summary element
    const summary = location.querySelector('summary');
    if (!summary) {
      this.addError(filePath, 'Location Structure', 'Location section should use <details> with <summary>', locationId);
    }

    // Check for location name
    const locationName = location.querySelector('.location-name');
    if (!locationName?.textContent?.trim()) {
      this.addError(filePath, 'Location Content', 'Location missing name element', locationId);
    }

    // Check for step counter
    const stepCounter = location.querySelector('.step-counter');
    if (!stepCounter) {
      this.addWarning(filePath, 'Location Features', 'Location missing step counter', locationId);
    }
    ...
    steps.forEach(step => {
      ...
      if (stepLocation && stepLocation !== locationId) {
        const stepId = step.getAttribute('data-step-id');
        this.addWarning(filePath, 'Location Consistency', `Step location "${stepLocation}" doesn't match parent location "${locationId}"`, stepId);
      }
    });
  }
  ...
  validateAccessibility(document, filePath) {
    ...
    if (!hasSkipToMain) {
      this.addWarning(filePath, 'Accessibility', 'Consider adding skip-to-content link for keyboard users');
    }

    // Check for heading structure
    const headings = document.querySelectorAll('h1, h2, h3, h4, h5, h6');
    if (headings.length === 0) {
      this.addWarning(filePath, 'Accessibility', 'No headings found - consider adding heading structure');
    } else {
      ...
      headings.forEach((heading, index) => {
        ...
        if (level - previousLevel > 1) {
          this.addWarning(filePath, 'Heading Structure', `Heading level jump from h${previousLevel} to h${level} - avoid skipping levels`);
        }
    ...
    if (liveRegions.length === 0) {
      this.addWarning(filePath, 'Accessibility', 'Consider adding aria-live regions for dynamic content announcements');
    }
    ...
    inputs.forEach(input => {
      ...
      if (!associatedLabel && !ariaLabel && !ariaLabelledby) {
        this.addError(filePath, 'Form Accessibility', `Input missing label association: ${inputId || input.type}`);
      }
    });
  ...
  printResults() {
    ...
    if (this.warnings.length > 0) {
      ...
      this.warnings.forEach(warning => {
        ...
        console.log(`   ${warning.file}: ${warning.category} - ${warning.message}${stepInfo}`);
      });
    }
  ...
  generateRecommendations() {
    ...
    if (errorCategories['Step Attributes']) {
      recommendations.push({
        ...
        priority: 'high',
        suggestion: 'Ensure all steps have required data attributes: data-step-id, data-category, data-location, data-tags'
      });
    }
    ...
    if (errorCategories['Form Accessibility'] || warningCategories['Form Accessibility']) {
      recommendations.push({
        ...
        priority: 'high',
        suggestion: 'Review form accessibility: ensure all inputs have proper labels and IDs match'
      });
    }
    ...
    if (warningCategories['Step Content']) {
      recommendations.push({
        ...
        priority: 'medium',
        suggestion: 'Review step text length - aim for clear, actionable instructions between 10-200 characters'
      });
    }
    ...
    if (warningCategories['Tag Count'] || warningCategories['Unknown Tag']) {
      recommendations.push({
        ...
        priority: 'medium',
        suggestion: 'Review tagging strategy - use 2-8 descriptive tags per step from the approved tag list'
      });
    }
    ...
    if (warningCategories['Heading Structure']) {
      recommendations.push({
        ...
        priority: 'medium',
        suggestion: 'Improve heading hierarchy - start with h1 and avoid skipping heading levels'
      });
    }
